...
class 窗口(QWidget):
    ...
    def 计算线程(self):
        ...
        if '极诣·剑魂' == self.角色属性A.实际名称:
            print("进入计算llllll~")

        if '极诣·狂战士' == self.角色属性A.实际名称:
            print("进入计算llllll~")
    ...
    def calc_done(self):
        setfile = open('./数据记录/{}-{}.csv'.format(self.角色属性A.实际名称, time.strftime('%Y-%m-%d-%H-%M-%S')), 'w',
                       encoding='gbk')
        for i in range(1):
            temp = ''
            for j in range(13):
                temp += str(self.排行数据[0][j]) + ','
            temp += self.角色属性A.实际名称 + ','
            temp += 装备列表[装备序号[self.排行数据[0][11]]].类型
            setfile.write(temp + '\n')
        self.close()
