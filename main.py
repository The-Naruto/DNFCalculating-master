...
import subprocess
import base64
from adventure_choice import NewMainForm
import csv_file_operation as cf
...
class 选择窗口(QMainWindow):
    ...
    def 打开窗口(self, name,show_flag=1):
        if show_flag == 1:
            ...
            module_name = "Characters." + name
            职业 = importlib.import_module(module_name)
            exec('window'+name+' = 职业.'+name+'()')
            exec('window'+name+'.hide()')
            exec('window' + name + '.计算()')
            exec('window' + name + '.hide()')
    ...
    def 检查更新(self):
        ...
        if self.网盘报错 == 1:
            ...
            box = QMessageBox(QMessageBox.Question, "提 示", "检测到新的计算器版本,是否更新？")
            box.setWindowIcon(self.icon)
            box.setStandardButtons(QMessageBox.Yes | QMessageBox.No | QMessageBox.Cancel)
            A = box.button(QMessageBox.Yes)
            B = box.button(QMessageBox.No)
...
class NewMain(NewMainForm):
    ...
    def calc(self):
        if '!' in self.textB.toPlainText():
            QMessageBox.question(self, "提示", "请先选择职业", QMessageBox.Yes, QMessageBox.Yes)
        else:
            startCalc(self.textB.toPlainText())

# 开始计算
def startCalc(adv):
    adventures = eval(adv)
    CHOOSE_ADVANTURES = adventures
    instance = 选择窗口()
    instance.hide()
    for adventure in adventures:
        # print(adventure)
        instance.打开窗口(adventure,0)
        time.sleep(5)

#     csv读取,合并,删除
    path = '数据记录'
    print("开始合并整合")
    # dir_file = cf.get_csvfile(path)
    # cf.merge_csvfile(path,dir_file)
